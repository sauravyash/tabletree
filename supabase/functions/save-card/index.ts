...
Deno.serve(async (req) => {
  ...
  try { ({ booking_id, setup_intent_id } = await req.json()); } catch { return json({ error: 'bad_request' }, 400, cors); }
  if (!booking_id || !setup_intent_id) return json({ error: 'missing_params' }, 400, cors);
  ...
  try {
    ...
    if (!result.ok) return json({ error: result.error }, 409, cors);
